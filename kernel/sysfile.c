...
sys_open(void)
{
	...
	if(omode & O_CREATE){
		...
		if (omode != O_NOFOLLOW) {
			int cycle_counter = 0;
			while(cycle_counter < 10 && ip->type == T_SYMLINK) {
				char linked[ip->size];
				ilock(ip);
				int n = readi(ip, linked, 0, ip->size);
				//cprintf("size %d\n", ip->size);
				linked[ip->size] = 0;
				if (n < 0) {
					//cprintf("1 %s\n", linked);
...
sys_symlink(void)
{
	...
	int n = writei(ip, path, 0, strlen(path));
	if (n == 0) {
		end_op();
		iunlock(ip);
		return -1;
